    ...
    <>
      <aside className="absolute flex">
        <div className={`px-[10px] pt-6 transition-colors duration-300 z-50 ${Object.entries(sidevarIsOpenState).find(([key, value]) => value)?.[0]}`} >
          <div className="flex flex-col gap-2 w-[163px] h-screen rounded-xl bg-[#F4F5F8] border-[1px] border-[#C5D3D3] p-2 shadow-inner">
            {Object.entries(sidevarIsOpenState).find(([key, value]) => value)?.[0] === "heading" && (
              <>
              {/* このブロックは後々のアップデートに備えてバックエンドから操作可能にする？？ */}
                <DraggableBlock id="heading1" title="見だし1" description="記事の見出しを作成するブロックです。" colorMode="heading" />
                <DraggableBlock id="heading2" title="見だし2" description="記事の見出しを作成するブロックです。" colorMode="heading" />
              </>
            )}
            {Object.entries(sidevarIsOpenState).find(([key, value]) => value)?.[0] === "text" && (
              <>
              </>
            )}
          </div>
